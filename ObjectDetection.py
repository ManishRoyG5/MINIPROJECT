...
def detectObject(CNNnet, total_layer_names, image_height, image_width, image, name_colors, class_labels,
    ...
    """
    Detect objects in the given image using the CNN model.
    """
    if detect:
        ...
        if (Boundingboxes is None or confidence_value is None or ids is None or class_ids is None):
            raise ValueError('[ERROR] Unable to draw boxes: One of the inputs is None.')
...
def listBoundingBoxes(image, image_height, image_width, threshold_conf):
    """
    Generate bounding boxes for detected objects based on the network's output.
    """
    box_array = []
    confidence_array = []
    class_ids_array = []
...
def displayImage(image):
    ...
    cv.waitKey(0)  # Change this to a specific delay if needed
